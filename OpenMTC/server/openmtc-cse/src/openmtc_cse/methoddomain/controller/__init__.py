...
from openmtc_onem2m.exc import (CSEOperationNotAllowed, STATUS_OK, CSETypeError,
                                ...
                                CSEContentsUnacceptable, CSETargetNotReachable, CSEMaxNumberOfMemberExceeded,CSEMemberTypeInconsistent)
from openmtc_onem2m.model import (ExpiringResource, Notification,
                                  ...
                                  DynAuthDasRequestC, SecurityInfo, SecurityInfoTypeE, AE,Group,MemberTypeE)
...
class OneM2MDefaultController(LoggerMixin):
    ...
    def _check_create_representation(self):
        ...
        self.logger.debug("What is RT %s ", self.resource_type)
        self.logger.debug("What is RT parent %s ", self.parent)
...
class RemoteCSEController(OneM2MDefaultController):
    ...
    def _handle_create(self):
        self.parent = self.resource
        del self.resource
        self.logger.debug("Creating Remote Controller +++++++++++++++++")
...
class ContainerController(OneM2MDefaultController):
    def _set_mandatory_create_attributes(self, values):
        super(ContainerController,
              self)._set_mandatory_create_attributes(values)
        self.logger.debug("Creating Container Controller +++++++++++++++++")

        self.logger.debug("test : ")
        self.logger.debug(values["maxNrOfInstances"])
...
class ContentInstanceController(OneM2MDefaultController):
    def _create_resource(self):
        ...
        self.logger.debug("When it will call ?????????")
...
class GroupController(OneM2MDefaultController):
    # memberIDs < maxNrOfMembers
    # reject MAX_NUMBER_OF_MEMBER_EXCEEDED
    # if dup remove old create group
    # check memberType != MIXED
    # loop check all member IDs INCLUDE sub-group by check each member ID
    ##in case member is virtual resource check resource type of parent
    # loop each member if is group
    # if memberType cannot retrieve due to lack privileage
    # return RECEIVER_HAS_NO_PRIVILEGE
    # if unreachable
    # memberTypeValidated = false and return result
    ### As soon as any wake up reciver perform validate memberType
    # originator know by subscribe?????
    # if consistencyStrategy is ABANDON_GROUP
    # delete group
    # if consistencyStrategy is ABANDON_MEMBER
    # delete member
    # if consistencyStrategy is SET_MIXED
    # set memberType = MIXED

    # loop validate all memberType or resourceType
    # validate success then memberTypeValidated == TRUE
    # validate fail then
    # if consistencyStrategy is ABANDON_ MEMBER
    # delete that member then memberTypeValidated == TRUE
    # if consistencyStrategy is ABANDON_GROUP
    # reject  Response Status Code = GROUP_MEMBER_TYPE_INCONSISTENT
    # if consistencyStrategy is SET_ MIXED
    # memberType = MIXED and memberTypeValidated = TRUE
    def _handle_create(self):
        self.logger.debug("Check point?1")
        self.parent = self.resource
        self.logger.debug("Check point?2")
        del self.resource
        self.logger.debug("Check point?3")
        self.now = datetime_now()
        self.fields = []
        self.logger.debug("Check point?4")
        values = self.request.content.get_values(True)
        self._removeDuplicate(values)
        self._checkNumberOfMember(values)
        self.logger.debug("Member Type %s ", values["memberType"])
        self._check_create_representation()
        self.logger.debug("Check point?5")
        test = self._create_resource()
        self.logger.debug("Can I get path here? %s ", test)
        self._memberTypeValidate(values)
        self.logger.debug("Check point?6")
        self._finalize_create()
        self.logger.debug("Check point?7")
        return self._send_create_response()


    def _checkNumberOfMember(self,values):
        if len(values["memberIDs"]) > values["maxNrOfMembers"]:
            raise CSEMaxNumberOfMemberExceeded()

    def _removeDuplicate(self,values):
        self.logger.debug(super)
        newList=[]
        for i in values["memberIDs"]:
            if i not in newList:
                newList.append(i)
        values["memberIDs"] = newList
        self.request.content.set_values(values)

    def _validateConS(self,values,memberList,index):
        self.logger.debug("validate S call : %s ",values["memberTypeValidated"] )
        if values["memberTypeValidated"] == False:
            self.logger.debug("memlist : %s ", memberList)
            self.logger.debug("index : %s ", index)
            if values["consistencyStrategy"] == "ABANDON_GROUP":
                raise CSEMemberTypeInconsistent()
            elif values["consistencyStrategy"] == "ABANDON_MEMBER":
                for member in index:
                    memberList.pop(member)
            elif values["consistencyStrategy"] == "SET_MIXED":
                values["memberTypes"] = "MIXED"
        self.logger.debug("new values : %s ", values["memberTypes"])
        self.request.content.set_values(values)
    def _memberTypeValidate(self,values):
        values["memberTypeValidated"] = True
        self.logger.debug("Strat memberTypeValidated : %s ", values["memberTypeValidated"])
        if values["memberType"] != 24:
            self.logger.debug("Strat requesting : %s ", values["memberType"])
            memberList = values["memberIDs"]
            unreachable = []
            for idx, member in enumerate(memberList):
              try:
                onem2m_response = self.api.handle_onem2m_request(
                      OneM2MRequest(
                          OneM2MOperation.retrieve, member, fr=self._abs_cse_id
                      )
                ).get()
                self.logger.debug("Testing Response : %s ", onem2m_response)
                self.logger.debug("Isinstance G : %s ", isinstance(onem2m_response.content,Group))
                self.logger.debug("Isinstance A3 : %s ", onem2m_response.content)
                self.logger.debug("Isinstance A4 : %s ", MemberTypeE(values["memberType"]).name)
                className = "%s" % MemberTypeE(values["memberType"]).name
                className = "%s%s" % (className[0].upper(),className[1:])
                self.logger.debug("Isinstance A2 : %s ", globals()[className])

                self.logger.debug("Isinstance A1 : %s ", isinstance(onem2m_response.content,globals()[className]))
                if isinstance(onem2m_response.content, Group):
                    self.logger.debug("In group : ")
                    self.logger.debug(onem2m_response.content.memberIDs)
                    ##### how to retrieve with value?
                    unreachable2=[]
                    memberListSubGroup = onem2m_response.content.memberIDs
                    for idy, member in enumerate(memberListSubGroup):
                        try:
                            onem2m_response2 = self.api.handle_onem2m_request(
                                OneM2MRequest(
                                    OneM2MOperation.retrieve, member, fr=self._abs_cse_id
                                )
                            ).get()
                            self.logger.debug("Isinstance SG1 : %s ", onem2m_response2.content)
                            if isinstance(onem2m_response2.content,Group):
                                pass
                            else:
                                unreachable2.append(idy)
                                values["memberTypeValidated"] = False
                        except OneM2MErrorResponse as e:
                             self.logger.error("CSE reported an error: %s ", e)
                             values["memberTypeValidated"] = False
                             unreachable2.append(idy)
                    ...
                    self.request.content.set_values(values)
                    self._validateConS(values,memberListSubGroup,unreachable2)

                elif isinstance(onem2m_response.content,globals()[className]):
                    self.logger.error("Member type match IDs: %s ", member)
                    self.logger.error("Why ?")
                else : values["memberTypeValidated"] = False
              except OneM2MErrorResponse as e:
                self.logger.error("CSE reported an error: %s ", e)
                values["memberTypeValidated"] = False
                unreachable.append(idx)
              except OpenMTCError as e:
                self.logger.error("Failed to reach the CSE: %s ", e)
                values["memberTypeValidated"] = False
                unreachable.append(idx)
              except Exception as e:
                self.logger.error("Unknown error: %s",e)
        ...
        self.request.content.set_values(values)
        self._validateConS(values,memberList,unreachable)
    ...
    def _handle_update(self):
        ...
        self.fields = []
         ...
         # loop each member if is group
          # if memberType cannot retrieve due to lack privileage
           # return RECEIVER_HAS_NO_PRIVILEGE
          # if unreachable ### As soon as any wake up reciver perform validate memberType
                                              # originator know by subscribe?????
           #set memberTypeValidated = false and return result
          ### As soon as any wake up reciver perform validate memberType
            # originator know by subscribe?????
           # if consistencyStrategy is ABANDON_GROUP
             # delete group
        ...
        #check if memberIDs > maxNrOfMembers or maxNrOfMembers < currentNrOfMembers
