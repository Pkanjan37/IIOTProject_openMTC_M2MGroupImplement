...
class XAE(LoggerMixin):
    ...
    def add_group_subscription(self, group, filter_criteria=None):
        """ Creates a Subscription to the ContentInstances of the given
        Container.

        :param group: Group object or group path string
        :param handler: reference of the notification handling function
        :param delete_handler: reference to delete handling function
        :param filter_criteria: (optional) FilterCriteria for the subscription
        """

        path = getattr(group, "path", group)

        # check if target is container
        if not isinstance(self.mapper.get(path), Group):
            raise RuntimeError('Target is not a group.')

        def handler_member_response(self, data):
            print()
    ...
    def create_group(self, target, group, labels=None):
        """ Creates a Container resource.
        ...
        if not isinstance(group,Group ):
            group = Group(resourceName=group)



        if group.expirationTime is None:
            group.expirationTime = self.get_expiration_time()
        ...
        try:
            group = self.mapper.create(path, group)
        except OneM2MErrorResponse as error_response:
            if error_response.response_status_code is STATUS_CONFLICT:
                c_path = path + '/' + group.resourceName
                group.path = c_path
                if (self.__resumed_registration and
                        c_path not in self.__known_containers):
                    container = self.mapper.update(container)
        ...
        self.__known_containers.add(group.path)
        self.__start_refresher(container, restore=restore_container)
        self.logger.info("Container created: %s." % (group.path,))
        return group
